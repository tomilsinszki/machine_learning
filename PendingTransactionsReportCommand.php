...
class PendingTransactionsReportCommand extends ContainerAwareCommand
{
    ...
    */
    ...
    private function loadUserIds() {
        ...
        $statement = $this->entityManager->getConnection()->prepare("SELECT u.id FROM account_user u WHERE u.locked=0 AND u.enabled=1 AND roles='a:0:{}' AND (MOD(u.id, 10) = 0) ORDER BY u.id ASC");
        $statement->execute();
    ...
    protected function execute(InputInterface $input, OutputInterface $output) {
        ...
        foreach ($this->periods as $periodName => $period) {
            foreach ($this->userIds as $userId) {
                ...
                $partnersStatement = $connection->prepare("SELECT p.id, p.main_category_id as category_id, c.parent_id as parent_category_id FROM Partner p LEFT JOIN Category c ON p.main_category_id=c.id WHERE p.`status`='active'");
                    ...
                    */
                    ...
                    foreach ($dates as $k2 => $date) {
                        ...
                        */
                        ...
                        $export[4][2][$k2] = $this->zScoreNumberOfPartnersSpentAtForMainCategory($connection, $mainCategoryId, $userId, $date['start'], $date['end']);
    ...
    */
    ...
    public function zScoreNumberOfPartnersSpentAtForMainCategory(Connection $connection, $mainCategoryId, $currentUserId, $start, $end) {
        $whereTerms = array();
        ...
        $queryText =
            "SELECT u.id u_id, COUNT(DISTINCT t.partner_id) p_count
            FROM cashback_transaction t
            LEFT JOIN account_user u ON t.user_id=u.id
            LEFT JOIN Partner p ON t.partner_id=p.id
            LEFT JOIN Category c ON p.main_category_id=c.id
